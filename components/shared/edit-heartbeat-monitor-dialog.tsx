...
import { editHeartbeatMonitorAction } from "@/lib/actions/monitors";
...
import { editHeartbeatMonitorSchema, EditHeartbeatMonitorInput } from "@/lib/validations";
...
interface EditHeartbeatMonitorDialogProps {
  monitor: Monitor;
...
export function EditHeartbeatMonitorDialog({
  monitor,
  planContext,
  alertChannels = [],
  selectedChannelIds = [],
  ...
  onOpenChange,
  onSuccess,
}: EditHeartbeatMonitorDialogProps) {
  const [isLoading, setIsLoading] = useState(false);
  const [channelIds, setChannelIds] = useState<string[]>(selectedChannelIds);

  const form = useForm<EditHeartbeatMonitorInput>({
    resolver: zodResolver(editHeartbeatMonitorSchema),
    defaultValues: {
      name: monitor.name,
      intervalMilliseconds: monitor.intervalMilliseconds,
      timeoutMilliseconds: monitor.timeoutMilliseconds,
      gracePeriodMilliseconds: monitor.gracePeriodMilliseconds,
  ...
  function validatePlanLimits(data: EditHeartbeatMonitorInput) {
  ...
  async function onSubmit(data: EditHeartbeatMonitorInput) {
    if (!validatePlanLimits(data)) return;

    setIsLoading(true);

    try {
      const result = await editHeartbeatMonitorAction(monitor.id, data);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>Edit Heartbeat Monitor</DialogTitle>
          <DialogDescription>
            Update the configuration for {monitor.name}. Changes take effect on the next check.
          </DialogDescription>
        </DialogHeader>

        {monitor.heartbeatUrl ? <HeartbeatUrlDisplay url={monitor.heartbeatUrl} /> : null}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="Nightly database backup" disabled={isLoading} {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
